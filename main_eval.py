...
# 설정 변수 (실험용)
# ----------------------------------
result_save_path = "/code/rppg metric/predictions"
test_dataset     = "PURE"    # ["UBFC", "PURE", "cohface"]
multiple_preds   = True      # False 면 기존 단일 모델 모드
model_names      = ["biMamba-blend","biMamba","bilstm","physdiff","LSTM","RNN","mixup","radiant","mamba","POS"]  # 비교할 모델 리스트
...
# 파일 경로 설정 & 로드
# ----------------------------------
label_fp = os.path.join(result_save_path, f"{test_dataset}_label_prediction.npy")
...
if multiple_preds:
    ...
    for m in model_names:
        p_fp = os.path.join(result_save_path, f"{test_dataset}_{m}_prediction.npy")
